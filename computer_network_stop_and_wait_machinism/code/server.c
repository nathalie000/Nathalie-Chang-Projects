...
int sendFile(FILE *fd)
{
    ...
    /*int send_base = 0;
    bool window_acked[4];
    for(int i=0;i<4;i++)window_acked[i]= false;
   ...
   //  for(int i=0;i<4;i++){
    ...
    snd_pkt.header.ack_num = n;
    snd_pkt.header.seq_num = n;
    sendto(sockfd, &snd_pkt, sizeof(snd_pkt), 0,(struct sockaddr *)&client_info, len);

     struct timeval timeout={4,100000};

    setsockopt(sockfd,SOL_SOCKET,SO_RCVTIMEO,(char*)&timeout,sizeof(struct timeval));
    if((recvfrom(sockfd, &rcv_pkt, sizeof(rcv_pkt), 0, (struct sockaddr *)&client_info, (socklen_t *)&len))== -1){
        resend = true;
        printf("Timeout! resend packet %d", snd_pkt.header.seq_num);
    }else{
        n++;
        printf("Received a packet (seq_num = %d, ack_num = %d)\n)",  rcv_pkt.header.seq_num, rcv_pkt.header.ack_num);
        if(rcv_pkt.header.isLast == 1) break;
    }
	//======================================
	// Checking timeout & Receive client ack
	//======================================
    ...
    while(dt<100){
        ...
        dt = (clock()*1000)/CLOCKS_PER_SEC - t;
        t = (clock()*1000)/CLOCKS_PER_SEC;
        //rcv ack

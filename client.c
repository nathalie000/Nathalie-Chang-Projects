...
//============
int sockfd = 0;
Udp_pkt snd_pkt,rcv_pkt; //net_buffer
...
int recvFile(FILE *fd)
{
	...
	printf("Receiving...\n");
	char buffer[123431]; //socket_buffer
	int index=0;
//	int window_cnt = 0;
	//char window_buffer[4][1024];
	//int window_is_last[4];
	...
	while(1)
	{
       ...
       /* if(rcv_pkt.header.seq_num == rcv_base ){ //window slide
            strcat(buffer, &rcv_pkt.data);
            if(rcv_pkt.header.isLast)ok_to_write = true;

            int i;
            for(i=1;i<=3;i++){
                if(window_buffer[i][0]!="\0"){
                    strcat(buffer, window_buffer[i]);
                    window_cnt -= 1;
                    if(window_is_last[i])ok_to_write = true;
                }else{
                    break;
                }
            }
            ...
            for(int j=0;j<4;j++){
                if(j+i < 4)
                    strcpy(window_buffer[j], window_buffer[j+i]);
                    window_is_last[j]=window_is_last[j+i];
                else
                    window_buffer[j][0]='\0';
                    window_is_last[j]=0;
            }
        }else{ //put in window
            strcpy(window_buffer[rcv_pkt.header.seq_num - rcv_base], &rcv_pkt.data);
            window_is_last[rcv_pkt.header.seq_num - rcv_base] = rcv_pkt.header.is_last;
        }*/
        ...
        if(rcv_pkt.header.is_last == 1){
            ...
            if(fwrite(buffer,sizeof(char),index,fd)){
               // printf("Failed to write from buffer to file.\n");
            }
            break;
        }
       ...
       // if(ok_to_write) printf("Client receivd finish\n");
	}
	fclose(fd);
	return 0;
}
